...
export default function VehicleDetailClient({ vehicle, similarVehicles, relatedPosts }: VehicleDetailClientProps) {
    ...
    <>
      <div className="min-h-screen bg-secondary-50">
        ...
        <div className="bg-white border-b border-secondary-100">
          <div className="container-custom px-4 md:px-6 py-4">
            <nav className="flex items-center gap-2 text-sm text-secondary-500">
              ...
              <span>/</span>
              <Link href="/vehiculos" className="hover:text-primary-600 transition-colors">Vehículos</Link>
              <span>/</span>
              <span className="text-secondary-800">{vehicle.title}</span>
            </nav>
          </div>
        </div>
        ...
        <div className="container-custom px-4 md:px-6 py-6 md:py-10">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-secondary-600">
                  Desde <span className="font-semibold text-secondary-800">{monthlyPayment}€/mes</span> con{' '}
                  <Link href="/financiacion" className="text-primary-600 hover:underline">financiación</Link>
                </p>
              </div>
          ...
          {/* Related Blog Posts Section - Internal linking for SEO */}
          {relatedPosts.length > 0 && (
            <div className="mt-16">
              ...
              <p className="text-secondary-600 mb-6">
                Consejos y guías que te pueden interesar sobre {vehicle.brand} y vehículos {vehicle.fuel.toLowerCase()}
              </p>
                ...
                {relatedPosts.map((post) => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      {post.extracto && (
                        <p className="text-xs text-secondary-500 line-clamp-2">{post.extracto}</p>
                      )}
                    </div>
                  </Link>
                ))}
          ...
          {/* SEO Content Section */}
          <div className="mt-16 bg-white rounded-xl p-6 md:p-8 border border-secondary-100">
            <h2 className="text-xl font-bold text-secondary-900 mb-3">
              {vehicle.brand} {vehicle.model} de segunda mano en Madrid
            </h2>
            <p className="text-secondary-600 text-sm leading-relaxed mb-3">
              Este <strong>{vehicle.title}</strong> del año <strong>{vehicle.year}</strong> está disponible en nuestro
              concesionario <strong>MID Car</strong> en Torrejón de Ardoz, Madrid. Con {formatKilometers(vehicle.km)} y
              motor {vehicle.fuel.toLowerCase()}, es una excelente opción para quienes buscan
              un <Link href={`/vehiculos?tipo=${vehicle.bodyType}`} className="text-primary-600 hover:underline">
            ...
            <p className="text-secondary-600 text-sm leading-relaxed">
              ...
              con plazos de hasta 10 años.{' '}
              <Link href="/contacto" className="text-primary-600 hover:underline">
                Contáctanos
              </Link>{' '}
              para más información o para concertar una visita.
            </p>
          </div>
        </div>
      </div>
