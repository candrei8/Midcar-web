const DEFAULT_MESSAGE = 'Hola, me gustaría más información sobre un vehículo de MID Car.'
...
export function WhatsAppFloat({ phone }: WhatsAppFloatProps) {
  const number = `34${phone.replace(/\D/g, '').replace(/^34/, '')}`
  ...
  return (
    <a
      ...
      className="fixed bottom-24 right-4 z-40 flex items-center gap-2 rounded-full bg-[#25D366] px-3.5 py-3 text-sm font-semibold text-white shadow-xl shadow-black/20 transition-transform hover:scale-105 md:right-5"
    >
