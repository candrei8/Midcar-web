...
export function VehicleDetailClient({ vehicle, similarVehicles }: VehicleDetailClientProps) {
  ...
  useEffect(() => {
    ...
    if (currentImageIndex >= safeImageCount) {
      setCurrentImageIndex(safeImageCount - 1)
    }
  }, [currentImageIndex, failedImages, lightboxOpen, vehicle])

  const monthlyPayment = vehicle.monthlyPayment || Math.round(vehicle.price / 60)
  const images = vehicle.images?.filter((image) => !failedImages.has(image)) || []
  const hasImages = images.length > 0
  const safeCurrentImageIndex = hasImages ? Math.min(currentImageIndex, images.length - 1) : 0
  const currentImage = hasImages ? images[safeCurrentImageIndex] : undefined
  ...
  return (
    <>
            ...
            <div className="space-y-6">
              ...
              {vehicle.label && labelColors[vehicle.label] && (
                <div className={cn(
                  ...
                  <span className="font-bold">{labelColors[vehicle.label].name}</span>
                </div>
              )}
      ...
      {hasImages && (
        <ImageLightbox
          images={images}
          initialIndex={safeCurrentImageIndex}
          isOpen={lightboxOpen}
